...
- [x] Cross-origin session cookie handling for production (Netlify + Render: SameSite=None, Secure=true, 	rust proxy=1)
- [x] Token encryption and decryption for stored OAuth refresh tokens
- [ ] Implement automatic token refresh retry logic when Google access tokens expire
...
- [x] Gemini 2.5 Flash integration for fast, structured JSON event parsing
...
- [x] AI-powered duplicate event detection (checking candidate events against existing calendar entries before creation)
...
- [x] Cron-based automated polling service (
ode-cron every 2 minutes)
...
- [x] Auto-adding extracted events to user's primary Google Calendar
